...
setupSurveyExcelSection = function(array) {
 ...
 if(Array.isArray(array)){//check if it arg is a valid array
 	...
 	ch_surv_data.datakeys=Object.keys(ch_surv_data.firstAssess.Data);//get array from key values of survey.Data object items
 	//surveydata_check(ch_surv_data.firstAssess,ch_surv_data.datakeys);
 	ch_surv_data.col_blk1 = ch_rawdata.getSecDataKeys(ch_surv_data.datakeys,'CHV2SEC1BLK1RW');//getSectionDataKeys for Section 1 
 	ch_surv_data.SecRowsArray= makeRowsArray(ch_surv_data.col_blk1, ch_surv_data.firstAssess);//get array of rows for Section 1 Data from datakeys
 	ch_surv_data.workbook_test = ch_rawdata.setFacilitySummaryExcel(workbook, ch_surv_data.firstAssess, ch_surv_data.SecRowsArray);
